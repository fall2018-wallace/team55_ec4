...
fD<-ggplot(sat)+geom_bar(mapping=aes(x=month,fill=Satisfaction),position="fill",width=0.4)+scale_fill_grey() + theme_classic()
...
fD1<-ggplot(sat)+geom_bar(mapping=aes(x=week_days,fill=Satisfaction),position="fill",width=0.4)+scale_fill_grey() + theme_classic()
fD1
#the Satisfaction for southeast is more on tuesdays
#the remaining are almost same for all days

fD2<-ggplot(sat)+geom_bar(mapping=aes(x=Airline.Name,fill=Satisfaction),position="fill",width=0.4)+scale_fill_grey() + theme_classic()

fD2
#the Satisfaction for different airlines is seen
#almost all of them have an average Satisfaction of 3.5

fD3<-ggplot(sat)+geom_bar(mapping=aes(x=Scheduled.Departure,fill=Satisfaction),position="fill",width=0.4)+scale_fill_grey() + theme_classic()
fD3
#the graph is more spread for southeast Airlines
#few of them lead to Satisfaction 3

fD4<-ggplot(sat)+geom_bar(mapping=aes(x=Departure.Delay.in.Minutes,fill=Satisfaction),position="fill",width=0.4)+scale_fill_grey() + theme_classic()
fD4
