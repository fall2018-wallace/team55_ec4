...
df<-data
#using this for Age
satisfactionAge<-ggplot(df)+geom_bar(mapping=aes(x=Age,fill=Satisfaction),fill="position",width=0.4)+scale_fill_grey() + theme_classic()
...
satisfactionAirlineStatus<-ggplot(df)+geom_bar(mapping=aes(x=Airline.Status,fill=Satisfaction),fill="position",width=0.4)+scale_fill_grey() + theme_classic()
...
satisfactionGender<-ggplot(df)+geom_bar(mapping=aes(x=Gender,fill=Satisfaction),fill="position",width=0.4)+scale_fill_grey() + theme_classic()

#similar trends when comparing south east and other airlines

#using this for price sensitivity
satisfactionPriceSensitivity<-ggplot(df)+geom_bar(mapping=aes(x=Price.Sensitivity,fill=Satisfaction),fill="position",width=0.4)+scale_fill_grey() + theme_classic()
...
satisfactionYearOfFirstFlight<-ggplot(df)+geom_bar(mapping=aes(x=Year.of.First.Flight,fill=Satisfaction),fill="position",width=0.4)+scale_fill_grey() + theme_classic()
